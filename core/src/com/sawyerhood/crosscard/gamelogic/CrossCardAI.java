...
public class CrossCardAI extends CrossCardPlayer {
	 ...
	 * 
	 * @param side
	 *            the orientation of the AI players, vertical or horizontal
	 * @param id
	 *            the id of the AI player
	 * @param difficulty
	 *            the difficulty setting for the AI, determines computation time
	 */
	...
	// drawn).
	/**
	 * Computes the actions to take during the AI's turn. Updates the board and
	 * deck with the changes
	 * 
	 * @param currentBoard
	 *            - the active board of the game
	 * @param currentDeck
	 *            - the active deck of the game
	 */
	public void turn(CrossCardBoard currentBoard, CrossCardDeck currentDeck) {
		...
		if (maxIndex == 9) {
			...
			// simulate playing the drawn card
			simulateLoop(fantasyBoard, fantasyDeck, getCurrentCard(), moves,
					false);
	...
	/**
	 * Simulates many cross card games and returns a move array whose max index represents the best move
	 ...
	 * @param playToReserve a boolean to determine if the reserve slot is a valid move for the simulation
	 */
	public void simulateLoop(CrossCardBoard fantasyBoard,
			CrossCardDeck fantasyDeck, CrossCard currentCard, int[] moves,
			boolean playToReserve) {
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					if (!fantasyBoard.isOccupied(i, j)) {
						...
						moves[i * 3 + j] += simulate(simulateBoard,
								simulateDeck);
					}
				}
			}
	...
	public static void aiTest() {
		...
		CrossCardAI ai1 = new CrossCardAI(CardType.VERTICAL, "player1",
				AIDifficulty.HARD);
		CrossCardAI ai2 = new CrossCardAI(CardType.HORIZONTAL, "player2",
				AIDifficulty.EASY);
